...
def add_system(
    ...
    with open(config_file_path, 'r') as config_file:
        try:
            cloud_config: CloudDict = yaml.load(config_file, Loader=yamlloader.ordereddict.CSafeLoader)['cloud']
    ...
    if (client_systems := cloud_config['client_systems']) is not None:
        taken_ports = [sys['port'] for sys in cloud_config['client_systems'].values() if sys['address'] == addr and sys['system_name'] == system_name]

        if system_name in client_systems:
            id += '-' + str(len(tuple(sys for sys in cloud_config['client_systems'].values() if sys['system_name'] == system_name))).rjust(3, '0')
        ...
        for sys in client_systems.values():
            if (sys['system_name'] == system_name and sys['address'] == addr and sys['port'] == port):
                raise ValueError(f'Client system with name "{system_name}", address {addr}, and port {port} already exists')
    ...
    cloud_config['client_systems'] = {key: cloud_config['client_systems'][key] for key in sorted(cloud_config['client_systems'])}
