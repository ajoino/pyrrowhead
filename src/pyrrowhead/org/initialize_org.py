...
def create_org_certificates(org_name: str, org_dir: Path, root_key, root_cert, password) -> KeyCertPair:
    org_cert_dir = org_dir / ORG_CERT_DIR / 'crypto'
    org_cert_dir.mkdir(parents=True)
    pair = generate_and_store_org_files(org_name, org_cert_dir, root_key, root_cert, password)
    return pair
...
def populate_org_dir(org_name: str, password: str):
    ...
    if not all(org_dir.joinpath(subdir).exists() for subdir in (ORG_CERT_DIR, ROOT_CERT_DIR)):
